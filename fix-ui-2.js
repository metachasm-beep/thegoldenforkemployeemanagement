...
const virtualizerSetup = `  const messagesEndRef = useRef<HTMLDivElement>(null);
...
if (endIdx === -1) {
  // Try with just \n
  const endMarker2 = '              <div ref={messagesEndRef} />\n            </div>';
  endIdx = content.indexOf(endMarker2, startIdx);
  if(endIdx !== -1) {
    endIdx += endMarker2.length;
  }
} else {
...
console.log("Successfully replaced rendering block! length:", newBlock.length);
