...
export default function ChatClientHacker({ currentEmployeeId, employees, initialConversations, isImpersonating = false }: ChatClientProps) {
  ...
  const EmployeeAvatar = ({ emp, className = "w-8 h-8" }: { emp: Employee, className?: string }) => {
    ...
    return (
      <div className="relative group shrink-0 mt-1 cursor-pointer z-10">
        ...
        <div className="absolute left-0 bottom-full mb-2 w-64 bg-black border border-[#00ff00] text-[#00ff00] rounded-none shadow-xl border border-gray-100 dark:border-gray-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 p-4 pointer-events-none">
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] bg-black text-[#00ff00] border border-[#00ff00] font-mono rounded-none overflow-hidden">
      <div className="w-80 border-r border-[#00ff00] flex flex-col bg-black border-r border-[#00ff00]">
        <div className="p-4 border-b border-[#00ff00]">
          <div className="relative">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-[#00aa00] text-xs" />
            <input 
              type="text" 
              placeholder="Search messages..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-black border border-[#00ff00] text-[#00ff00] pl-9 pr-4 py-2 rounded-none text-sm border border-gray-200 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-[#00ff00] bg-black"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto relative">
          {searchQuery ? (
            <div className="p-4 space-y-4">
              {isSearching ? <p className="text-sm text-[#00cc00] text-center">Searching...</p> : searchResults.length === 0 ? <p className="text-sm text-[#00cc00] text-center">No results found.</p> : searchResults.map(msg => (
                <div key={msg.id} className="bg-black border border-[#00ff00] text-[#00ff00] p-3 rounded-none border border-gray-100 dark:border-gray-700 cursor-pointer hover:border-amber-500" onClick={() => { setSearchQuery(''); setActiveConversationId(msg.conversationId); }}>
                  <p className="text-xs font-bold text-[#00cc00] mb-1">{msg.sender.name} in {msg.conversation?.name || 'DM'}</p>
            ...
            <>
              {conversations.map(c => {
                ...
                return (
                  <button key={c.id} onClick={() => setActiveConversationId(c.id)} className={`w-full text-left p-4 border-b border-[#00ff00] hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors flex items-center gap-3 ${activeConversationId === c.id ? 'bg-[#003300] text-[#00ff00] border-y border-[#00ff00]' : ''}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-[#00ff00] truncate">{displayTitle}</p>
                      {lastMsg && <p className="text-xs text-[#00cc00] truncate mt-0.5">{lastMsg}</p>}
                    </div>
                  </button>
                )
              })}
              <div className="p-4 pt-6">
                <h3 className="text-xs font-bold uppercase text-[#00aa00] text-xs mb-3 tracking-wider">Directory</h3>
      ...
      <div className="flex-1 flex flex-col bg-black relative">
        {activeConversationId ? (
          <>
            <div className="h-16 border-b border-[#00ff00] flex items-center px-6 bg-white/50 dark:bg-gray-900/50 backdrop-blur-md sticky top-0 z-20 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-[#00ff00]">{activeConvoDetails?.type === 'GROUP' ? activeConvoDetails.name : (otherParticipant?.name || 'Chat')}</h3>
              </div>
            </div>
                ...
                return (
                  <div key={msg.id} className={`flex gap-3 group ${isMe ? 'justify-end' : ''}`}>
                    ...
                    <div className={`max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col relative`}>
                      {showAvatar && !isMe && <span className="text-xs text-[#00cc00] mb-1 ml-1">{msg.sender?.name}</span>}
                      
                      {msg.parent && (
                        <div className="text-xs bg-black text-[#00ff00] border border-gray-200 dark:border-gray-700 rounded-lg p-2 mb-1 text-[#00cc00] truncate w-full flex items-center gap-1 opacity-70">
                      ...
                      <div className="relative flex items-center gap-2 group/msg">
                        {isMe && !isImpersonating && (
                          <div className="opacity-0 group-hover/msg:opacity-100 transition-opacity flex gap-1 bg-black border border-[#00ff00] text-[#00ff00] shadow-sm rounded-lg border border-gray-200 p-1 absolute right-full mr-2 top-0 z-10">
                            <button onClick={() => setReplyingTo(msg)} className="p-1 hover:bg-gray-100 rounded text-[#00cc00]"><Reply size={14}/></button>
                            <button onClick={() => toggleReaction(msg.id, '👍')} className="p-1 hover:bg-gray-100 rounded text-[#00cc00]"><Smile size={14}/></button>
                          </div>
                        )}
                        
                        <div className={`px-4 py-2.5 rounded-none prose prose-sm dark:prose-invert break-words max-w-full ${isMe ? 'bg-transparent text-[#00ff00] border-l-4 border-[#00ff00] pl-2' : 'bg-gray-100 dark:bg-gray-800 text-[#00ff00] rounded-bl-none'}`}>
                          <ReactMarkdown>{msg.content}</ReactMarkdown>
                        </div>

                        {!isMe && !isImpersonating && (
                          <div className="opacity-0 group-hover/msg:opacity-100 transition-opacity flex gap-1 bg-black border border-[#00ff00] text-[#00ff00] shadow-sm rounded-lg border border-gray-200 p-1 absolute left-full ml-2 top-0 z-10">
                            <button onClick={() => toggleReaction(msg.id, '👍')} className="p-1 hover:bg-gray-100 rounded text-[#00cc00]">👍</button>
                            <button onClick={() => toggleReaction(msg.id, '❤️')} className="p-1 hover:bg-gray-100 rounded text-[#00cc00]">❤️</button>
                            <button onClick={() => setReplyingTo(msg)} className="p-1 hover:bg-gray-100 rounded text-[#00cc00]"><Reply size={14}/></button>
                          </div>
                        )}
                      </div>
            ...
            <div className="p-4 bg-white dark:bg-gray-900 border-t border-[#00ff00] shrink-0 flex flex-col z-20">
              {replyingTo && (
                <div className="flex items-center justify-between bg-black text-[#00ff00] p-2 rounded-t-xl border-x border-t border-gray-200 dark:border-gray-700 text-sm">
                  <span className="text-[#00cc00] truncate flex-1"><Reply size={14} className="inline mr-1"/> Replying to {replyingTo.sender?.name}</span>
                  <button onClick={() => setReplyingTo(null)} className="text-[#00aa00] text-xs hover:text-gray-600"><X size={16}/></button>
                </div>
              )}
              {isImpersonating || activeConvoDetails?.isReadOnly ? (
                <div className={`flex-1 bg-black text-[#00ff00] border border-gray-200 dark:border-gray-700 px-4 py-3 text-[#00cc00] italic text-center text-sm ${replyingTo ? 'rounded-b-xl' : 'rounded-none'}`}>
                ...
                <form onSubmit={handleSend} className="flex gap-2">
                  <input 
                    ...
                    className={`flex-1 bg-black text-[#00ff00] border border-gray-200 dark:border-gray-700 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#00ff00] bg-black text-[#00ff00] ${replyingTo ? 'rounded-b-xl' : 'rounded-none'}`}
                  />
                  <button type="submit" disabled={!inputText.trim() || loading} className="bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white px-6 rounded-none font-medium transition-colors">Send</button>
                </form>
              )}
            </div>
          </>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center text-[#00aa00] text-xs">
            <div className="w-16 h-16 rounded-none bg-black text-[#00ff00] flex items-center justify-center mb-4"><span className="text-2xl">💬</span></div>
