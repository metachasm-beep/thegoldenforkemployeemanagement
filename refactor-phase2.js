...
// We need to inject useVirtualizer into ChatClientSoft
...
// We need to inject Skeletons for loading state and replace the mapping logic.
// Find the <div className="flex-1 overflow-y-auto..."> block
const regex = /<div className="flex-1 overflow-y-auto p-4 md:p-6 pb-8 space-y-6 flex flex-col z-10 custom-scrollbar">([\s\S]*?)<div ref=\{messagesEndRef\} \/><\/div>/;

const newVirtualizerBlock = `
<div ref={parentRef} className="flex-1 overflow-y-auto p-4 md:p-6 pb-8 z-10 custom-scrollbar relative">
              ...
              <div className={\`max-w-[70%] \${isMe ? 'items-end' : 'items-start'} flex flex-col relative\`}>
                ...
                <div className="relative flex items-center gap-2 group/msg">
                  {isMe && !isImpersonating && (
                    <motion.div initial={{opacity:0, scale:0.9}} whileHover={{scale:1.05}} animate={{opacity:0}} whileInView={{opacity:1}} className="opacity-0 group-hover/msg:opacity-100 transition-all flex gap-1 bg-white/80 dark:bg-gray-800/80 backdrop-blur shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 p-1 absolute right-full mr-2 top-0 z-10">
                  ...
                  {!isMe && !isImpersonating && (
                    <motion.div initial={{opacity:0, scale:0.9}} whileHover={{scale:1.05}} animate={{opacity:0}} whileInView={{opacity:1}} className="opacity-0 group-hover/msg:opacity-100 transition-all flex gap-1 bg-white/80 dark:bg-gray-800/80 backdrop-blur shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 p-1 absolute left-full ml-2 top-0 z-10">
                    ...
                    <div className="flex -space-x-1">
                      {readReceipts.slice(0, 3).map((r: any) => (
                        <div key={r.id} className="relative w-4 h-4 rounded-full border border-white dark:border-gray-900 z-10 overflow-hidden">
                          {r.avatarUrl ? (
                            <img src={r.avatarUrl} alt={r.name} className="object-cover w-full h-full" />
                          ) : (
...
content = content.replace(regex, newVirtualizerBlock);

// Remove the old useEffect that scrolls, since virtualizer handles it now
content = content.replace(
  /useEffect\(\(\) => \{\s*messagesEndRef\.current\?\.scrollIntoView\(\{ behavior: 'smooth' \}\);\s*\}, \[messages\]\);/,
  ""
);

// We need to replace ReactMarkdown inside the default output, since we added components
content = content.replace(/<ReactMarkdown>\{msg\.content\}<\/ReactMarkdown>/g, "<ReactMarkdown components={MarkdownComponents}>{msg.content}</ReactMarkdown>");
